...
function buildBatchExplanation(batch: BatchWithBlocks, style: AIResponseStyle) {
  ...
  const latestActor = getBlockActor(latest, batch);
  const latestLocation = getBlockLocation(latest, batch);
  const latestNarrative = getBlockNarrative(latest);
  ...
  return `${summary} It started on ${formatDateTime(genesis.timestamp)} and the most recent record was logged on ${formatDateTime(latest.timestamp)}. Latest note: ${latestNarrative}.`;
}
...
function buildDashboardMessage(
  ...
  return `${intro} The most recent activity is on batch ${latestBatch.batch_id}, where the latest event is ${toTitleCase(latestBatch.last_event_type ?? "origin record")} at ${latestBatch.last_timestamp ? formatDateTime(latestBatch.last_timestamp) : "an unconfirmed time"}.`;
}
